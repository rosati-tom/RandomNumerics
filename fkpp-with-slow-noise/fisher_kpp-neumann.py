...
class fkpp:
	...
	def do_step(self):
		...
		self.state_a = self.slv.apply_rslv( self.state_a - (np.multiply(np.multiply( self.state_a, self.state_a - 1), self.noise))*delta_t )

		self.state_b = self.slv.apply_rslv(self.state_b - (np.multiply(np.multiply( self.state_b, self.state_b - 1), self.noise))*delta_t )
...
fkpp_sample = fkpp(x_0, x_1, tau, sigma, delta_t, delta_x, space_pts)
...
ani       = animation.FuncAnimation(fig, animate, frames=100000, interval = 50, blit = True)

ani.save('fkpp.mp4', bitrate = 40000)

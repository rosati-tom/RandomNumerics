...
# Solve the Fisher ODE with time-varying noise
...
class 1D_ode:
	...
	def __init__(self, x_0, x_1, tau, sigma, delta_t):
		...
		self.noise =  np.random.normal(size = 1 , scale = sigma)
	...
	def do_step(self):
		...
		self.state_a = self.state_a + (self.state_a - self.state_a**2)*self.noise*delta_t

		self.state_b = self.state_b + (self.state_b - self.state_b**2)*self.noise*delta_t
